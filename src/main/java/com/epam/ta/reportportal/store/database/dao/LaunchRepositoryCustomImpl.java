...
public class LaunchRepositoryCustomImpl implements LaunchRepositoryCustom {
	...
	@Override
	public Page<Launch> findLatest(Filter filter, Pageable pageable) {

		List<Launch> launches = dsl.fetch(QueryBuilder.newBuilder(filter).with(pageable).build()).map(LAUNCH_MAPPER);
